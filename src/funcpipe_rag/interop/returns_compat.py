"""Module 09 Core 2: optional `returns` interop (end-of-Module-09).
...
from funcpipe_rag.result.types import NONE, Err, ErrInfo, Ok, Option, Result, Some
...
def to_result(obj: Any, *, map_exc: Any | None = None) -> Result[Any, ErrInfo]:
    """Convert a `returns.result.Result` into `funcpipe_rag.result.types.Result`.
    ...
    try:
        ...
        if obj.__class__.__name__ == "Success":
            return Ok(getattr(obj, "_inner_value"))
        if obj.__class__.__name__ == "Failure":
            inner = getattr(obj, "_inner_value")
...
def to_option(obj: Any) -> Option[Any]:
    """Convert a `returns.maybe.Maybe` into `funcpipe_rag.result.types.Option`."""
    ...
    try:
        if obj.__class__.__name__ == "Some":
            return Some(getattr(obj, "_inner_value"))
