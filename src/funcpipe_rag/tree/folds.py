"""Tree folds and scans as safe recursion (end-of-Module-07)."""
...
def scan_tree(tree: TreeDoc, seed: R, combiner: Callable[[R, TreeDoc, int, Path], R]) -> Iterator[R]:
...
def fold_count_length_maxdepth(tree: TreeDoc) -> tuple[int, int, int]:
    def step(acc: tuple[int, int, int], node: TreeDoc, depth: int, path: Path) -> tuple[int, int, int]:
        del path
