"""Streaming combinators for Result-valued pipelines (end-of-Module-05)."""
...
def split_results_to_sinks(xs: Iterable[Result[T, E]], on_ok: Callable[[T], None], on_err: Callable[[E], None]) -> None:
    for r in xs:
        if isinstance(r, Ok):
            on_ok(r.value)
        else:
