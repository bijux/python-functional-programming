"""Module 09 Core 5: config-driven, stateless pipelines (end-of-Module-09).
...
from funcpipe_rag.rag.stages import clean_doc, embed_chunk, iter_chunk_doc
...
def _env_from_params(params: Mapping[str, object]) -> RagEnv:
    ...
    if not isinstance(chunk_size, int) or not isinstance(overlap, int) or not isinstance(tail_policy, str):
...
def build_rag_pipeline(
    ...
    *,
    ...
    for step in config.steps:
        ...
        if overlap:
            raise ValueError(f"Artifact overlaps config params for step {step.name}: {sorted(overlap)}")
